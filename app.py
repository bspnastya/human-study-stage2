...
import streamlit as st, gspread, sys
...
def make_qs():
    ...
    corners=[{"group":g,"alg":a,"img":url(g,a),"qtype":"corners",
              ...
              "correct":CORNER[g]}
             for g,a in itertools.product(GROUPS,ALGS_COR) if cnt.get((g,a),0)<TARGET_SHOWS]
    seq=letters+corners
    random.shuffle(seq)
    ...
    def writer():
        ...
        while True:
            ...
            if buf:
                ...
                except:gpass=[json.dumps(r,ensure_ascii=False) for r in buf];buf.clear()
...
if not st.session_state.name:
    ...
    <p style="color:#111;">Для начала теста введите любой псевдоним и нажмите Enter или нажмите «Сгенерировать псевдоним».</p>""",unsafe_allow_html=True)
...
if st.session_state.phase=="intro":
    st.markdown(f"{q['prompt']}",unsafe_allow_html=True)

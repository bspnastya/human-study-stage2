...
MOBILE_QS_FLAG="mobile"; BASE_URL="https://storage.yandexcloud.net/test3123234442"
TIME_LIMIT=15; TARGET_SHOWS=21
...
WITH_CHARS=["img1_dif_corners","img2_dif_corners","img4_same_corners","img5_same_corners"]
NO_CHAR="img3_same_corners_no_symb"
...
components.html(f"""
<script>
(function(){{const f='{MOBILE_QS_FLAG}',m=innerWidth<1024;
...
if st.experimental_get_query_params().get(MOBILE_QS_FLAG)==["1"]:
    st.markdown("""
    <style>body{background:#808080;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;}h2{font-size:1.3rem;font-weight:500;line-height:1.4;}</style>
...
def render_timer(sec:int,tid:str):
    ...
    <script>
      (function(){{let t={sec};
        const s=document.getElementById('t{tid}');
        const i=setInterval(()=>{{if(--t<0){{clearInterval(i);return;}}
        if(s)s.textContent=t;}},1000);}})();
    </script>""",height=50)
...
<b>правый верхний и левый нижний</b>, и определить, окрашены ли они в одинаково с точностью до оттенка.<br><br>Картинка будет доступна в течение <b>15&nbsp;секунд</b>. Время на ответ не ограничено."""
...
пробелами, запятыми и т.&nbsp;д., а также слитное написание.<br><br>Если букв нет — нажмите кнопку <b>«Не вижу букв»</b>."""
...
def finish(ans):
    ...
    Q.put([datetime.datetime.utcnow().isoformat(),st.session_state.name,q["№"],
    ...
    st.session_state.update(idx=st.session_state.idx+1,phase="intro",
                            phase_start_time=None,_timer_flags={}); st.experimental_rerun()

...
st.markdown("""<style>html,body,.stApp,[data-testid="stAppViewContainer"],.main,.block-container{background:#808080!important;color:#111!important;}h1,h2,h3,h4,h5,h6,p,label,li,span{color:#111!important;}header[data-testid="stHeader"]{display:none;}.stButton>button{min-height:52px!important;padding:0 20px!important;border:1px solid #555!important;background:#222!important;color:#ffffff!important;border-radius:8px!important;}.stButton>button:hover{background:#333!important;color:#ffffff!important;}.stButton>button:focus{background:#333!important;color:#ffffff!important;box-shadow:none!important;}.stButton>button:disabled{background:#444!important;color:#888!important;cursor:not-allowed!important;opacity:0.6!important;}.stButton>button>div,.stButton>button>div>p{color:inherit!important;}input[data-testid="stTextInput"]{height:52px;padding:0 16px;font-size:1.05rem;}</style>""",unsafe_allow_html=True)
...
def make_qs()->List[Dict]:
    ...
    random.shuffle(qs);[q.__setitem__("№",i) for i,q in enumerate(qs,1)];return qs
...
if q["qtype"]=="corners":
    ...
    with col:
        ...
        if st.button("Не вижу букв",key=f"none{st.session_state.idx}",disabled=has_letters):
            if has_letters:st.error("Очистите поле ввода, если не видите букв.")
            else:finish("Не вижу")
    if txt and re.fullmatch(r"[А-Яа-яЁё ,.;:-]+",txt):finish(txt.strip())
    elif txt and not re.fullmatch(r"[А-Яа-яЁё ,.;:-]+",txt):st.error("Допустимы только русские буквы и знаки пунктуации.")

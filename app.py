...
def render_timer(sec:int,tid:str):
    ...
    <script>
      ...
      const iv=setInterval(()=>{{if(--t<0){{clearInterval(iv);return;}}
      if(span)span.textContent=t; }},1000); }})();
...
if st.session_state.idx>=len(st.session_state.questions):
    st.markdown("<div style='margin-top:50px;padding:40px;text-align:center;"
                "font-size:2rem;color:#fff;background:#262626;border-radius:12px;'>"
...
if q["qtype"]=="corners":
    ...
    txt=st.text_input(q["prompt"],key=f"t{st.session_state.idx}",
                      placeholder="Введите буквы и нажмите Enter")
    col,_=st.columns([1,3])
    with col:
        btn_disabled=bool(re.search(r"[А-Яа-яЁё]", txt or ""))
